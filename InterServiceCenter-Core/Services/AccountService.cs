...
public class AccountService
{
    ...
    public async Task<JsonResponse> SaveProfilePhoto(AddAttachmentDTO account, string loggedEmail)
    {
        ...
        var createdImageName = await _fileService.SaveProfilePhoto(account.ImageFile, allowedFileExtentions);
    ...
    public async Task<JsonResponse> ModifyProfilePhoto(AddAttachmentDTO account, string loggedEmail)
    {
        ...
        var existingFileName = checkIfAccountExist.ProfilePhotoFile;
        var createdImageName =
            await _fileService.ModifyProfilePhoto(account.ImageFile, allowedFileExtentions, existingFileName);
    ...
    public JsonResponse RemoveProfilePhoto(int id, string loggedEmail)
    {
        ...
        var deletionResponse = _fileService.DeleteProfilePhoto(checkIfAccountExist.ProfilePhotoFile);
    ...
    public JsonResponse ModifyAccount(ModifyAccountDTO account, string loggedEmail)
    {
        ...
        if (checkIfEmailExist != null && checkIfEmailExist.Id != account.Id)
            return new JsonResponse
                { StatusCode = 404, Message = "ERROR: Email address already in use in another account." };
    ...
    public JsonResponse DeleteAccount(int id, string loggedEmail)
    {
        ...
        if (accountToDelete.Role == "Super Administrator")
            return new JsonResponse
                { StatusCode = 401, Message = "ERROR: You can't delete a Super Administrator account." };
