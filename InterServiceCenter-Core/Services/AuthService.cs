...
public class AuthService
{
    ...
    public JsonResponse LoginAccount(string email, string password, bool remember)
    {
        ...
        if (account == null)
        {
            return new JsonResponse{ StatusCode = 404, Message = "ERROR: Email and/or password not found in our records."};
        }
        
        // Verify hashed password on the database
        bool verifyPassword = BCrypt.Net.BCrypt.Verify(password, account.HashedPassword);
    ...
    public JsonResponse SaveAccount(string firstName, string lastName, string email, string password, string confirmPassword)
    { 
        
        if (_dbContext.IscAccounts.Any(emp => emp.Email == email))
        {
            // Email already exist on database
            return new JsonResponse{ StatusCode = 400, Message = "ERROR: Email " + email + " already exist in our records."};
        }
