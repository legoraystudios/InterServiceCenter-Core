...
public class JwtToken
{
    ...
    public JwtToken(IConfiguration configuration, InterServiceCenterContext dbContext, IHttpContextAccessor httpContextAccessor, GeneralUtilities utilities)
    {
    ...
    public bool VerifyBearer(ClaimsIdentity identity)
    {
        ...
        if (verifySession == null || verifySession.Email != emailClaim?.Value)
        {
            return false;
        }
        ...
        // then failed the authorization.
        if (ipAddressClaim?.Value != _utilities.GetClientIpAddress())
        {
            return false;
        }
    ...
    {
        // Get Email Address assigned to the token
        var emailClaim = context.Claims.First(c => c.Type == "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress").Value;
